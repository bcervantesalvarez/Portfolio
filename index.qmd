...
map <- leaflet() %>%
  addTiles() %>%
  setView(lng = -119.095377, lat = 46.235109, zoom = 12)
...
# Add JavaScript code to activate the map only on hover
htmlwidgets::onRender(map, '
  function(el, x) {
    el.addEventListener("mouseenter", function(e) {
      x.map.invalidateSize();
    });
  }
')
...
map <- leaflet() %>%
  addTiles() %>%
  setView(lng = -123.033121, lat = 44.9391565, zoom = 12)
